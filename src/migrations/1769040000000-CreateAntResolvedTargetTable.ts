...
export class CreateAntResolvedTargetTable1769040000000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_ant_resolved_target_category`
    )
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_ant_resolved_target_status`
    )
    await queryRunner.query(`DROP TABLE IF EXISTS "ant_resolved_target"`)
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."ant_resolved_target_category_enum"`
    )
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."ant_resolved_target_status_enum"`
    )
  }
}
